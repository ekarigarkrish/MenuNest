...
export default function SettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Section className="flex h-full min-h-[calc(100vh-64px)] rounded-2xl overflow-hidden my-6 sm:my-0 w-full bg-white border border-gray-200 shadow-sm relative">
      ...
      {/* Sidebar Navigation */}
      <aside className={`absolute lg:relative z-50 lg:z-10 h-full w-80 shrink-0 bg-gray-50 flex flex-col transition-transform duration-300 ease-in-out left-0 top-0 bottom-0 border-r border-gray-200 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="p-6 pb-4 flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-gray-900 flex items-center gap-2 mb-0 lg:mb-6">
            ...
            {SETTINGS_LINKS.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setIsSidebarOpen(false)}
                  className={`w-full text-left flex items-center justify-between p-3.5 my-1 rounded-xl transition-all duration-200 border group ${isActive
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-lg border ${isActive ? "bg-cayenne-red-50 border-cayenne-red-100" : "bg-gray-100 border-gray-200/50"} shrink-0`}>
      ...
      {/* Main Content Area */}
      <main className="flex-1 h-full flex flex-col overflow-hidden bg-white relative">
        <div className="lg:hidden p-4 border-b border-gray-100 flex items-center justify-between bg-white z-30 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-4 sm:p-6 lg:p-10">
          <motion.div
            key={pathname}
