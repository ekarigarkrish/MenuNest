...
const schema = yup
  .object({
    name: yup.string().required("Name is required").strict(),
    slug: yup.string().required("Slug is required").strict(),
    categoryId: yup.string().required("Category is required").strict(),
    price: yup.number().required("Price is required").positive("Price must be positive").strict(),
    discount: yup.number().min(0).max(100).optional().strict(),
    discountPrice: yup.number().optional().strict(),
    prepTime: yup.number().min(0).optional().strict(),
    description: yup.string().optional().strict(),
    isAvailable: yup.boolean().default(true).strict(),
    isFeatured: yup.boolean().default(false).strict(),
    isVegetarian: yup.boolean().default(false).strict(),
    image: yup.mixed<File>().nullable().optional().strict(),
  })
  .strict();
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const parsedVal = type === "number" ? (val === "" ? undefined : parseFloat(val)) : val;
                field.onChange(parsedVal);
                if (onChangeOverride) onChangeOverride(e, field);
              }}
              className={`w-full ${prefix ? "pl-8" : "px-4"} ${suffix ? "pr-8" : "pr-4"} py-2.5 ${
                props.readOnly ? "bg-gray-50/50 cursor-not-allowed" : "bg-white"
              } border ${
                error ? "border-red-500 focus:ring-red-500/10 focus:border-red-500" : "border-gray-200 focus:ring-cayenne-red-500/10 focus:border-cayenne-red-500"
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        reset({
          name: "", slug: "", categoryId: activeCategoryId || "", price: undefined,
          discount: undefined, discountPrice: undefined, prepTime: undefined,
          description: "", isAvailable: true, isFeatured: false, isVegetarian: false, image: null,
        });
      }
    }
    ...
    <Modal
      ...
      footer={
        <div className="flex gap-3 w-full sm:w-auto">
          ...
          <Button className="rounded-xl w-full sm:w-auto shadow-md shadow-cayenne-red-500/20 hover:shadow-lg hover:shadow-cayenne-red-500/30" onClick={handleSubmit((d) => createMenuItem(d))} isLoading={isCreating || isSubmitting}>
            Save Item
          </Button>
        </div>
      }
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      error={errors.categoryId?.message} placeholder="Select Category"
                    />
                  )}
                />
              </div>
                  ...
                  <>
                    <textarea
                      ...
                      className={`w-full px-4 py-3 bg-white border ${
                        errors.description ? "border-red-500 focus:ring-red-500/10 focus:border-red-500" : "border-gray-200 focus:ring-cayenne-red-500/10 focus:border-cayenne-red-500"
